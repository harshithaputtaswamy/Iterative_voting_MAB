...
for _ in range(iterations):
    borda_scores_arr.append(train(iterations_per_batch, batch, 1, 0.5, parsed_soc_data))   # for each iteration returns a dictionary containing voter and there fnal reward i.e borda score of top candidate

borda_scores_arr = np.array(borda_scores_arr)
avg_borda_score_arr = borda_scores_arr.sum(axis=0)
avg_borda_score_arr = avg_borda_score_arr/iterations

num_iter_arr = [i for i in range(50, iterations_per_batch + 1, batch)]
...
plot = plt.figure()
plt.plot(num_iter_arr, avg_borda_score_arr, "r")
