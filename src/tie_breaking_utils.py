...
from read_result import get_tie_preferences
...
file_to_read = "setting_1_approval_test_config_test_3_approval_count_2_voter_10_cand_5_committee_size_3_iter_100000_avg_50.json"
file_to_read = curr_dir + "/numerical_results/tie_breaking_rules_study/" + voting_rule + "/" + file_to_read

preferences, run_setting = get_tie_preferences(file_to_read)
...
tie_breaking_rule_list = run_setting["tie_breaking_rule_list"]

kt_dict_interval = {}
for test in config.keys():
	kt_dict_interval[test] = {}
...
for i in range(iterations, end_interval, -window_size):
	...
	for test in kt_dict.keys():
		for tie_breaking_rule in kt_dict[test].keys():
			if tie_breaking_rule not in kt_dict_interval[test]:
