...
voting_rule_list = ["borda", "borda_top_cand", "approval", "plurality", "anti_plurality", "chamberlin_courant"]
...
os.makedirs(curr_dir + "/graph_results/voting_rules_comp_study/", exist_ok=True)
os.makedirs(curr_dir + "/numerical_results/voting_rules_comp_study/", exist_ok=True)
...
# for key in input_conf.keys():
output_file = 'setting_{}_test_config_{}_tie_breaking_rule_{}_approval_count_{}_voter_'.format(voting_setting, key, tie_breaking_rule, approval_count) + str(num_voters) + '_cand_' + str(num_candidates) + \
...
output_path = os.path.join(curr_dir + "/numerical_results/voting_rules_comp_study/", output_file)

for voting_rule in voting_rule_list:
    
    test_details[voting_rule] = {}
    ...
    result["graph_coords"][voting_rule] = {}
    ...
    test_details[voting_rule] = run_details
...
plt.ylabel("Avg Borda Satisfaction")
...
if voting_rule == "approval":
    ...
    graph_file = 'avg_score_setting_{}_test_config_{}_tie_breaking_rule_{}_voter_'.format(voting_setting, key, tie_breaking_rule) + str(num_voters) + '_cand_' + str(num_candidates) + \
                "_committee_size_" + str(committee_size) + "_iter_" + str(iterations) + "_avg_" + str(avg_runs) + '.png'
graph_path = os.path.join(curr_dir + "/graph_results/voting_rules_comp_study/", graph_file)
...
plot = plt.figure()
for voting_rule in voting_rule_list:
    plt.plot(num_iter_arr, result["graph_coords"][voting_rule]["avg_num_ties"], label=voting_rule)

plt.legend(loc='upper right')
plt.xlabel("Number of iterations")
plt.ylabel("Avg Number of ties")
# plt.ylim(0, actual_highest_vote_per_approval + 1)
plt.show()
if voting_rule == "approval":
    graph_file = 'avg_ties_setting_{}_test_config_{}_tie_breaking_rule_{}_approval_count_{}_voter_'.format(voting_setting, key, tie_breaking_rule, approval_count) + str(num_voters) + '_cand_' + str(num_candidates) + \
                "_committee_size_" + str(committee_size) + "_iter_" + str(iterations) + "_avg_" + str(avg_runs) + '.png'
else:
...
result["run_setting"] = {
    ...
    "batch" : batch,
    "voting_rule_list" : voting_rule_list,
